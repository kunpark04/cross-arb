...
- `depth_at_fire.py` — **does DISPLAYED cross-venue depth at fire predict the FILL? (the council's go/no-go)** from `bot-rs/executions.jsonl`: groups the 178 fires (book→approved→submit→fire_outcome), buckets locks vs misses by `depth_c2` at fire, **stratified by category** (depth_c2 is Simpson-confounded — sports deep+aborts, weather thin+fills). `--selftest`. **Verdict 2026-06-16: WORLD 2 — the book is PHANTOM** — per-category AUC ≤0.57 (sports 0.42, weather 0.57), stratified-perm p=0.64, gate lift ≤3.3pts, conversion MONOTONE-DECREASING in depth (11.9%→0% @≥5000); Monte-Carlo power 89-100% ⇒ evidence of absence, not under-power. Depth-gate scale lever REFUTED → [0027]; the locks that complete ARE profitable (capacity problem, not a losing strategy). (`stats-ml-logic-reviewer` reproduced SOUND, 0 CRITICAL)
