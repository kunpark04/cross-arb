...
function Show-Alert([string]$msg) {
    ...
    Write-Warning "cross-arb DOWN — $msg"
    Set-Content -LiteralPath $AlertFile -Value "[$stamp] cross-arb collector ALERT`n$msg`nhost: $RemoteHost"
    try {                                                   # best-effort desktop balloon (no module needed)
        ...
        $ni.ShowBalloonTip(15000, 'cross-arb collector DOWN', $msg, [System.Windows.Forms.ToolTipIcon]::Warning)
        Start-Sleep -Seconds 2; $ni.Dispose()
    } catch {
        try { if (Get-Module -ListAvailable BurntToast) { Import-Module BurntToast; New-BurntToastNotification -Text 'cross-arb collector DOWN', $msg } }
