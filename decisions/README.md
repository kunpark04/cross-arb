...
| [0013](0013-econ-grid-step-twin-and-measurement-integrity.md) | Econ joins on the grid-step TWIN (`≥T` ↔ `>T−step`; old pairing = off-by-one phantom edges, data quarantined) + measurement integrity: detection-time CLOSE stamps, ws_reconnect censoring markers, single-subscription Kalshi invariant, degraded-discovery prune skip | Accepted |
