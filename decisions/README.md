...
| [0020](0020-pmus-first-serial-plus-recovery-cost-gate.md) | **pmus-first serial execution** (resolve the slow/uncertain pmus leg before opening the fast Kalshi leg ⇒ no naked fast leg; cancel the resting GTC pmus order on abort) + **recovery-cost gate** (skip arbs whose pmus bid↔ask spread exceeds the edge) — from the first live arb's naked leg (−11¢) | Accepted (owner-directed; from the first live round-trip) |
| [0021](0021-fill-or-kill-orders-and-churn-cooldown.md) | **FILL-OR-KILL entry orders** (unfilled = killed at the venue, never rests ⇒ `filled:false` is TERMINAL, no late-fill race — the 2026-06-15 untracked-naked-position incident) + per-slug **churn cooldown** + ambiguous-leg **fail-close** + dry-run exec-log split | Accepted (from a live incident; pmus FOK enum DEMO-gated before re-arm) |
| [0022](0022-live-edge-floor-1.5c.md) | Lower the **LIVE bot edge floor 2.0 → 1.5¢** (owner a-priori risk-appetite under 0015; net-of-fees floor) — the 0014 pre-registered τ=2¢ stays the **research-path** value, untouched; Kalshi `fill_or_kill` API-doc-verified (optional enum, unknown→400) | Accepted; floor value **superseded by 0023** (reverted to 2.0¢ — no demo, verify FOK live first); analysis + FOK verification stand |
| [0023](0023-revert-live-edge-floor-to-2c.md) | **Revert the live edge floor 1.5 → 2.0¢** — no demo access ⇒ the FOK kill-confirm must be done LIVE; run the first live FOK verification at the conservative pre-registered 2¢ floor (supersedes 0022's value; keeps its analysis + the Kalshi FOK API-doc verification) | Accepted (owner-directed) |
| [0024](0024-partial-fill-flatten-any-non-clean-lock.md) | **Partial-fill handling** — pmus ignores FOK (runs IOC, partial-fills); a non-zero fill that isn't a clean both-FULL lock is FLATTENED (every `fill_qty>0` leg unwound at its own qty, atomic, fail-close, both legs). Supersedes 0021's "FOK makes `filled:false` terminal" for pmus | Accepted (live incident; 2 coding-agent + 1 adversarial review → SAFE_TO_ARM; Kalshi float-count WARN) |
| [0025](0025-dynamic-fire-order.md) | **Dynamic fire-order** — fire the THINNER leg first (per-leg ask-volume) so its FOK-reject is a clean abort, never a naked commit (extends 0020's pmus-first; recovery now flattens EITHER venue). Exposed + fixed 0024's Kalshi-float-count WARN | Accepted (owner-directed after 3 live execution-order pushes; adversarial-reviewed SAFE_TO_ARM; live-verified miahigh) |
| [0026](0026-maker-mode-not-viable-prereg-threshold.md) | **Maker mode NOT_VIABLE on current data** — measured `p_hedge` 36%/46% < ~53% breakeven, EV negative (the Kalshi maker fills on the flow that empties pmus → naked → recovery loss; it inverts 0025's safety). Don't build; PRE-REGISTER the forward go-threshold (bootstrap p_hedge CI lower-bound clears breakeven on ≥6 fresh clusters). A stats-review caught a clock-skew look-ahead that had flipped the sign | Accepted (measure-first; design scoped + shelved, re-measurable free) |
| [0027](0027-depth-gate-refuted-pmus-book-phantom.md) | **Depth-gate scale lever REFUTED; the displayed cross-venue book is PHANTOM at retail size** — 178-fire replay (`depth_at_fire.py`): depth-at-fire does NOT predict the fill (per-category AUC ≤0.57, stratified-perm p=0.64, conversion monotone-DECREASING in depth, 89-100% power ⇒ evidence of absence). Don't scale on a depth gate / breadth / bigger clips; bot stays a 1-contract rig. Locks that complete ARE profitable (capacity problem). One gap pre-registered: instrument the pmus-side depth ladder before the FINAL "uncapturable" verdict | Accepted (council-framed, `stats-ml-logic-reviewer` reproduced SOUND/0-CRITICAL; complements 0025+0026) |
| [0028](0028-live-bot-on-droplet-override-0007.md) | **Live bot on the droplet (override of 0007's no-trade-key invariant)** — run the bot perpetually on the always-on droplet at a **3-contract cap**; stages the trade-capable Kalshi key + pmus secret on the box (a separate confined `cross-arb-bot` user keeps the monitor's read-only isolation intact). Linux binary built in WSL Ubuntu-24.04 (rustls → glibc-only); logs mirror to `Kalshi/data/cross-arb-bot/`. The secret hand-off is owner-run by hard platform rule ([L45]) | Accepted (owner-explicit override, like 0015; DEPLOYED + LIVE 2026-06-16, all configs enabled) |
