...
- **Stream resilience** (2nd review C6) — each venue WS runs in a supervised reconnect-with-backoff loop, so a
  clean or abnormal close re-subscribes the current targets instead of silently half-killing the collector;
  the health beacon carries per-venue `rx_age` so a wedged-but-connected stream is observable off-box.
