...
| 13 | [allocation-policy-2026-06-10.md](allocation-policy-2026-06-10.md) | **Clip-stage allocation, tested OOS** — FIFO loses; the lever is a **2¢ edge floor + a deploy-to-full per-pair cap** (~+64–269% over FIFO out-of-sample, 11 diversified pairs); clip-cap-alone is risk-control not PnL (−3%); the "wait 1 s + sort" idea captures only 3.6% of the gap. Records the phantom that was 75% of the in-sample headline + the `capturable()` fix ([0012], [L20]). **Method demo on 0.81 d.** Scripts: `alloc_policy_experiment.py`/`clip_threshold_test.py` |
