...
| 12 | [execution-feasibility-2026-06-09.md](execution-feasibility-2026-06-09.md) | **Read-only empirical tests** — latency (~86–261ms, network-bound), shadow leg-fill (hit-rate collapses with latency; **corrected 0013**: leg-fail 55.5% @1s / 62.7% @2s once the debouncer flush-stamp lag is removed; ~27% die ~instantly — **sub-second read added 2026-06-11**: naked 17–23% @100–150 ms, taker not rejected at real RTT), settlement reconciliation (pmus finalization-lag; the "unreliable interim" claim **retracted 2026-06-11 — parse bug [L23]**, see #16), adverse-selection (instrumented). Probes: `latency_probe`/`shadow_fill`/`settle_recon`/`adverse_selection.py` |
...
| 15 | [fee-pin-2026-06-10.md](fee-pin-2026-06-10.md) | **Fees re-pinned from primary sources** — all 4 coefficients confirmed (Kalshi taker via the CFTC-filed schedule; pmus via docs.polymarket.us/fees + live `feeCoefficient`); the real finding: **Kalshi maker fees exist only on `quadratic_with_maker_fees` series — all 5 weather series charge makers $0** (and pmus rebates −0.0125), inverting the fee wall for the queued maker-side study. `/series/fee_changes` = live tripwire, currently empty |
| 16 | [probe-program-2026-06-11.md](probe-program-2026-06-11.md) | **All 10 ranked next-steps probed in one read-only pass** (first ~16 h of post-0013 data + live APIs) — taker NOT rejected at measured RTT (naked 17–23% @100–150 ms); maker study narrows to **rest-on-Kalshi + taker-hedge-pmus** (+0.14–0.44¢/attempt bounded; rest-on-pmus toxic); **weather settlement recon CLOSED (360/360, 3-way vs NWS CLI — pmus sibling-array parse bug found+fixed, [L23])**; **MLB void window is minutes** (47–90 min; 5-min statsapi poll detects, unwind +12–13¢ on trigger); early-exit = hold-all; recycle arm $0; cluster cap = risk-knob; ladder/trade logging + fee tripwire built → next gated redeploy. Scripts: `maker_feasibility`/`early_exit_ev`/`probe_mlb_postpone`/`recycle_arm_experiment` + extended `shadow_fill`/`adverse_selection`/`settle_recon` |
