...
use risk::{evaluate, evaluate_triple, Exposure};
use types::*;
...
/// Shared, mutable WC 3-leg-basket state (parallel to `PairState`, for the Dutch-book path). `by_game` is
/// the authoritative `TripleMeta` per game; `by_key` indexes EVERY per-outcome slug AND Kalshi ticker (6
/// keys per game) back to the game, so a book frame on any of a game's 6 markets finds its triple. The
/// books themselves are the SAME `pmus_books`/`kalshi_books` the 3 per-outcome binary pairs already
/// subscribe — a triple adds NO subscription, only this routing index.
#[derive(Default)]
struct TripleState {
    by_game: std::collections::HashMap<String, TripleMeta>,
    by_key: std::collections::HashMap<String, String>, // outcome slug OR Kalshi ticker -> game
}

impl TripleState {
    fn insert(&mut self, m: TripleMeta) {
        for oc in &m.outcomes {
            self.by_key.insert(oc.slug.clone(), m.game.clone());
            self.by_key.insert(oc.ticker.clone(), m.game.clone());
        }
        self.by_game.insert(m.game.clone(), m);
    }
    fn remove(&mut self, game: &str) {
        if let Some(m) = self.by_game.remove(game) {
            for oc in &m.outcomes {
                self.by_key.remove(&oc.slug);
                self.by_key.remove(&oc.ticker);
            }
        }
    }
}
...
async fn run_live(cfg: &Config, backend: std::sync::Arc<dyn ExecutionBackend>, creds: std::sync::Arc<venue::VenueCreds>) {
    ...
    let (initial, initial_triples) = match discovery::discover(&http).await {
        Ok(d) => {
            println!(
                "[discovery] {} weather + {} econ + {} sports + {} world-cup pairs tracked ({} total subscribable); {} world-cup 3-leg baskets",
                d.weather_pairs, d.econ_pairs, d.sports_pairs, d.soccer_pairs, d.pairs.len(), d.triples.len()
            );
            report_coverage(&d);
            (d.pairs, d.triples)
        }
    ...
    // WC 3-leg DUTCH-BOOK routing state (parallel to `pairs`). The triples' per-outcome slugs/tickers are
    // ALREADY in pm_tracked/k_tracked (the 3 per-outcome binary pairs of the same game registered them above),
    // so the basket needs NO extra subscription — only this game-routing index + the live books. Shared with
    // the refresh task (which rebuilds it from each re-discovery) so new WC games appear mid-session (L7).
    let triple_state: Arc<Mutex<TripleState>> = Arc::new(Mutex::new(TripleState::default()));
    {
        let mut ts = lock(&triple_state);
        for t in initial_triples {
            ts.insert(TripleMeta::from(t));
        }
    }
    // HELD baskets keyed by game (the exposure caps bind across the session; a partial-fill recovery flattens
    // the filled legs). `triple_pending` is the in-flight de-dup (a game with a spawned-but-unacked basket).
    let mut triples_held: HashMap<String, TriplePosition> = HashMap::new();
    let mut triple_pending: HashSet<String> = HashSet::new();
    ...
    // the 3-leg BASKET outcome channel (parallel to outcome_tx): a spawned submit_triple reports its
    // TripleAck back here. Kept separate so the 2-leg SubmitOutcome stays byte-unchanged.
    let (triple_tx, mut triple_rx) = tokio::sync::mpsc::unbounded_channel::<TripleSubmitOutcome>();
    ...
    loop {
        ...
        let ev = tokio::select! {
            ...
            t = triple_rx.recv() => {
                if let Some(out) = t {
                    apply_triple_outcome(&backend, &mut triples_held, &kalshi_books, &pmus_books, &mut exposure, &mut triple_pending, &mut flattening, &outcome_tx, &halt, out);
                }
                continue;
            }
            _ = heartbeat.tick() => {
        ...
        // ---- 3-LEG DUTCH-BOOK (WC) evaluation, PARALLEL to the per-outcome binary above. This frame's
        //      outcome slug may belong to a tracked WC game; if so, ALSO price the full-game basket on the
        //      complete 6-book state. Independent of the binary path: a game can lock a basket AND/OR its
        //      per-outcome binaries. Same in-flight + halt + one-position-per-game de-dup as the 2-leg path.
        let game = lock(&triple_state).by_key.get(&slug).cloned();
        if let Some(game) = game {
            evaluate_triple_frame(
                cfg, &backend, &triple_state, &game, &kalshi_books, &pmus_books, &mut exposure,
                &mut triple_pending, &triples_held, &flattening, &halt, &triple_tx,
            );
        }
    }
    if halt.load(Ordering::Relaxed) {
...
async fn refresh_loop(
    ...
    triple_state: std::sync::Arc<std::sync::Mutex<TripleState>>,
    ...
    loop {
        ...
        // WC 3-leg DUTCH-BOOK index refresh (parallel to the pair add/prune below). The basket's books ARE
        // the per-outcome pairs' books (handled below), so this only updates the game-routing index: on a
        // complete catalog REBUILD it (adds new WC games, drops settled ones — a settled-and-held basket
        // settles to cash on its own; WC has no postpone-unwind source, so no held-basket prune guard is
        // needed, unlike the moneyline W16 case); on a TRUNCATED catalog only ADD (an off-page game must not
        // drop a live basket). New keys are already subscribed via the per-outcome pairs.
        {
            let mut ts = lock(&triple_state);
            if prune_ok {
                *ts = TripleState::default();
                for t in &fresh.triples {
                    ts.insert(TripleMeta::from(t.clone()));
                }
            } else {
                for t in &fresh.triples {
                    if !ts.by_game.contains_key(&t.game) {
                        ts.insert(TripleMeta::from(t.clone()));
                    }
                }
            }
...
// ====================================================================================================
// 3-LEG DUTCH-BOOK (World Cup) — a PARALLEL path. None of the 2-leg builders above (`plan_legs`,
// `build_legs`, `position_from_intents`, `fire_legs`, the recovery helpers) are touched. The 3 outcome
// legs each buy YES on the venue `dutch_book` chose as cheapest, priced from that venue's book.
// ====================================================================================================

/// Static metadata for a tracked WC 3-leg Dutch-book game (parallel to `LivePair`): the per-game id, the
/// three outcomes' venue-native ids + tags + pmus tick/min-qty, the cluster, the settle flag, and the
/// days-to-game. The LIVE books are looked up separately (keyed by slug/ticker) — the SAME books the three
/// per-outcome binary pairs already subscribe, so a triple needs no extra subscription wiring.
#[derive(Clone, Debug)]
struct TripleMeta {
    game: String, // pair identity = the pmus game key (`<a>-<b>-<date>`)
    outcomes: [TripleLeg; 3],
    cluster: String,
    settle_clean: bool,
    days_to_event: Option<f64>,
}

/// One outcome's static identity inside a [`TripleMeta`]: the tag (A/D/B), its pmus slug + Kalshi ticker
/// (the books are keyed on these), and the pmus per-market order constraints the leg builder needs.
#[derive(Clone, Debug)]
struct TripleLeg {
    tag: OutcomeTag,
    slug: String,   // pmus market slug (pmus book key)
    ticker: String, // Kalshi ticker (Kalshi book key)
    pm_min_tick: Option<f64>,
    pm_min_qty: Option<f64>,
}

impl From<discovery::TripleSpec> for TripleMeta {
    fn from(t: discovery::TripleSpec) -> Self {
        TripleMeta {
            game: t.game,
            cluster: t.cluster,
            settle_clean: t.settle_clean,
            days_to_event: t.days_to_event,
            outcomes: t.outcomes.map(|o| TripleLeg {
                tag: o.tag,
                slug: o.slug,
                ticker: o.ticker,
                pm_min_tick: o.pm_min_tick,
                pm_min_qty: o.pm_min_qty,
            }),
        }
    }
}
...
impl TripleMeta {
    /// Build the live 3-outcome `SoccerTriple` quote (the cross-venue books per outcome + the cross-outcome
    /// MIN depth on the chosen-cheapest legs). `None` if ANY outcome lacks BOTH books (incomplete — wait for
    /// the missing snapshot, never trade a partial basket). `dutch_book` then prices the basket from it.
    fn build_quote(
        &self,
        kalshi_books: &std::collections::HashMap<String, book::KalshiBook>,
        pmus_books: &std::collections::HashMap<String, book::PmusBook>,
    ) -> Option<SoccerTriple> {
        let mut outcomes = [TripleOutcome::default(); 3];
        // collect each outcome's CHEAPEST-venue YES-ask ladder (the ladder we'd lift to buy that leg).
        let mut ladders: [Vec<(f64, f64)>; 3] = [Vec::new(), Vec::new(), Vec::new()];
        for (i, leg) in self.outcomes.iter().enumerate() {
            let pm = pmus_books.get(&leg.slug)?; // no pmus book yet -> incomplete basket
            let k = kalshi_books.get(&leg.ticker)?; // no Kalshi book yet -> incomplete basket
            outcomes[i] = TripleOutcome { tag: leg.tag, q: OutcomeQuote { tag: leg.tag, pm: pm.touch(), k: k.touch() } };
            ladders[i] = cheaper_yes_ask_ladder(pm, k);
        }
        Some(SoccerTriple {
            game: self.game.clone(),
            outcomes,
            cluster: self.cluster.clone(),
            settle_clean: self.settle_clean,
            depth: basket_depth(&ladders),
            days_to_event: self.days_to_event,
        })
    }
}

/// The CHEAPEST venue's YES-ask ladder (ascending by price) for one outcome — the ladder we'd lift to buy
/// that leg. Picks the venue with the lower BEST ask (ties -> pmus); an empty/one-sided book yields an
/// empty ladder (-> the basket has 0 depth at that leg, so it can't lock — correct).
fn cheaper_yes_ask_ladder(pm: &book::PmusBook, k: &book::KalshiBook) -> Vec<(f64, f64)> {
    let use_pm = match (pm.best().1, k.best().1) {
        (Some(p), Some(kk)) => p <= kk,
        (Some(_), None) => true,
        (None, Some(_)) => false,
        (None, None) => return Vec::new(),
    };
    if use_pm { pm.yes_ask_ladder() } else { k.yes_ask_ladder() }
}

/// BASKET DEPTH — fillable contract-baskets while the MARGINAL basket (the 3 current ask levels summed)
/// keeps the gross Dutch-book edge `1 - sum(3 levels)` at `>= {2c, 1c, 0c}`. The 3-leg analogue of
/// `book::depth_curve` (the 2-leg cross-venue walk): a 3-pointer walk over the three chosen-venue YES-ask
/// ladders that lifts the min available qty across the 3 best levels each step, accumulating into the
/// {2c,1c,0c} buckets while the marginal basket edge clears each threshold, and STOPS as soon as the
/// marginal basket would cost `>= $1` (edge < 0). This is what bounds the basket size correctly — summing
/// each leg's whole ladder would over-state depth (deep levels push the basket past the lock). An empty
/// leg ladder -> 0 depth (the basket can't lock without all three).
fn basket_depth(ladders: &[Vec<(f64, f64)>; 3]) -> Depth {
    let thresholds = [0.02_f64, 0.01, 0.0];
    let mut acc = [0.0_f64; 3];
    let mut idx = [0usize; 3];
    let mut rem = [
        ladders[0].first().map(|&(_, q)| q).unwrap_or(0.0),
        ladders[1].first().map(|&(_, q)| q).unwrap_or(0.0),
        ladders[2].first().map(|&(_, q)| q).unwrap_or(0.0),
    ];
    loop {
        // all three legs must still have a level — a basket needs ALL three outcomes.
        if (0..3).any(|j| idx[j] >= ladders[j].len()) {
            break;
        }
        let edge = 1.0 - (0..3).map(|j| ladders[j][idx[j]].0).sum::<f64>();
        if edge < thresholds[2] {
            break; // the marginal basket now costs >= $1 -> no more lockable depth
        }
        let step = rem[0].min(rem[1]).min(rem[2]);
        if step <= 1e-9 {
            break;
        }
        ...
        for j in 0..3 {
            ...
            if rem[j] <= 1e-9 {
                idx[j] += 1;
                rem[j] = ladders[j].get(idx[j]).map(|&(_, q)| q).unwrap_or(0.0);
            }
        }
    }
    Depth { c2: acc[0].round() as u32, c1: acc[1].round() as u32, c0: acc[2].round() as u32 }
}

/// Plan the THREE Dutch-book legs for a priced basket: each outcome's YES on the venue `dutch_book` chose
/// (its `legs[i].venue`), the limit price read from THAT venue's book (the cheapest YES ask the signal
/// used), the venue-native market id (pmus slug for a pmus leg, Kalshi ticker for a Kalshi leg), tag
/// A/D/B, coid `xarb3-<game>-<tag>`. A pmus leg quantizes UP to its tick (W1 toward-marketable) and is
/// SKIPPED (whole basket -> None) if `size` is below the market's min qty (a sub-min order would reject,
/// leaving a partial basket — fail safe). `None` if any leg can't be priced/rounds out of 1..=99c.
fn build_triple(meta: &TripleMeta, q: &SoccerTriple, sig: &signal::DutchSignal, size: u32) -> Option<[OrderIntent; 3]> {
    let mut out: Vec<OrderIntent> = Vec::with_capacity(3);
    // zip the three parallel per-outcome arrays (static leg, chosen venue, live books) by index.
    for ((leg, dleg), oc) in meta.outcomes.iter().zip(sig.legs.iter()).zip(q.outcomes.iter()) {
        let chosen = dleg.venue; // the venue dutch_book picked as cheapest for this outcome
        // the YES-ask price from the CHOSEN venue's book in this quote (the exact price the signal summed).
        let (market, raw_price, pm_tick, pm_min_qty) = match chosen {
            Venue::Pmus => (leg.slug.clone(), oc.q.pm.yes_ask?, leg.pm_min_tick, leg.pm_min_qty),
            Venue::Kalshi => (leg.ticker.clone(), oc.q.k.yes_ask?, None, None),
        };
        // a pmus leg below the market min qty would reject -> skip the WHOLE basket (a fired partial basket
        // is the naked-pair case this gate prevents); then quantize UP to the pmus price tick (W1).
        let price = if chosen == Venue::Pmus {
            if let Some(min_qty) = pm_min_qty {
                if (size as f64) < min_qty {
                    return None;
                }
            }
            quantize_to_tick(raw_price, pm_tick, Action::Buy)
        } else {
            raw_price
        };
        let pc = cents(Some(price), Action::Buy)?; // entries BUY -> ceil (stays marketable, W1)
        out.push(OrderIntent {
            venue: chosen,
            market,
            action: Action::Buy,
            side: Side::Yes, // a Dutch book buys YES on every outcome
            price_cents: pc,
            qty: size,
            client_order_id: format!("xarb3-{}-{}", meta.game, leg.tag.as_char()),
        });
    }
    Some([out.remove(0), out.remove(0), out.remove(0)])
}

/// W6 analogue for the basket: recompute the REALIZED net from the three ROUNDED leg prices + the void
/// tail and confirm it still clears the edge floor (and the basket costs < $1). Each leg rounds to a whole
/// cent independently (eroding up to +1c of cost per leg), so the booked basket can be thinner than the
/// gated `sig.net`; without this the fired basket could fall under the floor. Mirrors `realized_edge_clears_floor`.
fn realized_basket_clears_floor(cfg: &Config, legs: &[OrderIntent; 3], void_tail: f64) -> bool {
    use crate::ledger::{marginal_taker_fee, KALSHI_TAKER_COEF, PMUS_TAKER_COEF};
    let leg_fee = |leg: &OrderIntent| {
        let p = leg.price_cents as f64 / 100.0;
        if !(0.0 < p && p < 1.0) {
    ...
    let cost = legs.iter().map(|l| l.price_cents as f64 / 100.0).sum::<f64>();
    if cost >= 1.0 {
        return false; // a >= $1 basket pays more than the $1 payout — never book it
    }
    let realized: f64 = round4((1.0 - cost) - legs.iter().map(leg_fee).sum::<f64>() - void_tail);
    realized * 100.0 >= cfg.edge_floor_cents
}

/// The held `TriplePosition` from a both-filled basket entry: build straight from the three entry
/// `OrderIntent`s (each leg = its venue/market/side=YES; the pair `market` = the game key). The 3-leg
/// unwind SELLs back the same (venue, market, side). `pm_tick` is recorded ONLY on a pmus leg (W2 — the
/// recovery/unwind SELL floor-quantizes to it); Kalshi legs carry `None`. The per-outcome pmus tick comes
/// from `meta` keyed by the leg's market (slug).
fn triple_position_from_intents(meta: &TripleMeta, legs: &[OrderIntent; 3]) -> TriplePosition {
    let pm_tick_for = |market: &str| {
        meta.outcomes.iter().find(|o| o.slug == market).and_then(|o| o.pm_min_tick)
    };
    TriplePosition {
        ...
        legs: std::array::from_fn(|i| PositionLeg {
            ...
            market: legs[i].market.clone(),
            side: legs[i].side,
            venue_order_id: String::new(), // filled from the fill ack in the outcome handler
            pm_min_tick: if legs[i].venue == Venue::Pmus { pm_tick_for(&legs[i].market) } else { None },
        }),
        size: legs[0].qty,
...
/// The result of a SPAWNED `submit_triple`, reported back to the loop so all position/exposure bookkeeping
/// runs on the loop's own turn (parallel to `SubmitOutcome` for the 2-leg path; that is unchanged). Named
/// distinctly from `types::TripleOutcome` (the per-outcome BOOK pair) — this is the SUBMISSION message.
struct TripleSubmitOutcome {
    game: String,
    ack: exec::TripleAck,
    /// the basket position to RECORD if all 3 legs filled (else the reservation is released).
    position: TriplePosition,
    legs: [OrderIntent; 3], // the entry legs (needed to price each filled leg's recovery flatten)
    meta: TripleMeta,       // per-outcome pmus tick (for the recovery SELL floor) + the books are looked up live
    cost_per: f64,
}

/// The WC void/postpone-tail cost per $1 used to haircut the Dutch-book net (the basket must clear the
/// edge floor NET of this tail). Small + conservative — the per-outcome binary pairs absorb the same tail
/// implicitly via `settle_clean`. A const for now (no env knob); stage-3 can calibrate it against the
/// measured void rate. `0.0` would treat the basket as tail-free (the per-outcome pairs' implicit stance).
const WC_VOID_TAIL: f64 = 0.005;

/// Evaluate ONE tracked WC game's 3-leg DUTCH BOOK on the current complete 6-book state, and SPAWN the
/// basket if it locks. Mirrors the per-frame 2-leg block: build the live quote (all 6 books present, else
/// skip), price it (`dutch_book`), de-dup (halt / in-flight / already-held), gate it (`evaluate_triple`),
/// re-check the realized basket clears the floor from the rounded leg prices (W6), reserve exposure, mark
/// in-flight, and spawn `submit_triple`. A no-arb / unpriceable / gated-out basket is silently skipped (the
/// per-outcome binaries of the same game are evaluated independently in the 2-leg block).
#[allow(clippy::too_many_arguments)]
fn evaluate_triple_frame(
    cfg: &Config,
    backend: &std::sync::Arc<dyn ExecutionBackend>,
    triple_state: &std::sync::Arc<std::sync::Mutex<TripleState>>,
    game: &str,
    ...
    pmus_books: &std::collections::HashMap<String, book::PmusBook>,
    exposure: &mut Exposure,
    triple_pending: &mut std::collections::HashSet<String>,
    triples_held: &std::collections::HashMap<String, TriplePosition>,
    flattening: &std::collections::HashSet<String>,
    halt: &std::sync::atomic::AtomicBool,
    triple_tx: &tokio::sync::mpsc::UnboundedSender<TripleSubmitOutcome>,
) {
    // RUNTIME-HALT + in-flight + already-held de-dup BEFORE pricing (one basket per game; a re-entry while
    // held would stack a second reservation against a single tracked basket).
    if halt.load(std::sync::atomic::Ordering::Relaxed)
        || triple_pending.contains(game)
        || flattening.contains(game)
        || triples_held.contains_key(game)
    {
        return;
    }
    // clone the game's meta out so we don't hold the triple_state lock across the book locks / the spawn
    // (the refresh task may be rebuilding the map concurrently).
    let Some(meta) = lock(triple_state).by_game.get(game).cloned() else { return };
    // build the complete 6-book quote; incomplete (a missing outcome book) -> wait.
    let Some(q) = ({
        let kb = lock(kalshi_books);
        meta.build_quote(&kb, pmus_books)
    }) else { return };
    let sig = signal::dutch_book(&[q.outcomes[0].q, q.outcomes[1].q, q.outcomes[2].q], WC_VOID_TAIL);
    if sig.no_arb {
        return; // not a lockable basket this frame
    }
    // affordable baskets from the remaining total-notional room at this basket cost.
    let room = (cfg.max_total_notional - exposure.total).max(0.0);
    let affordable = (room / sig.basket_cost.max(0.01)).floor().max(0.0) as u32;
    if let Ok(a) = evaluate_triple(cfg, &q, &sig, exposure, affordable) {
        let Some(legs) = build_triple(&meta, &q, &sig, a.size) else { return }; // one-sided leg -> skip
        if !realized_basket_clears_floor(cfg, &legs, WC_VOID_TAIL) {
            return; // rounding eroded the basket under the floor -> don't fire
        }
        println!(
            "[live] TRIPLE ENTRY {game}  size={}  net={:.1}c @ {:.2}c/$-day  basket_cost={:.2}",
            a.size, a.net * 100.0, a.edge_rate, a.cost_per
        );
        let pos = triple_position_from_intents(&meta, &legs);
        reserve_triple_exposure(exposure, &pos, a.cost_per);
        triple_pending.insert(game.to_string());
        spawn_submit_triple(backend, triple_tx, game.to_string(), legs, pos, meta.clone(), a.cost_per);
    }
}

/// SPAWN a `submit_triple` off a cloned `Arc<backend>`, reporting the `TripleAck` back over `triple_tx`.
/// Mirrors `spawn_submit` (the 2-leg spawn): the event loop NEVER fires the 3-leg POST inline.
#[allow(clippy::too_many_arguments)]
fn spawn_submit_triple(
    backend: &std::sync::Arc<dyn ExecutionBackend>,
    triple_tx: &tokio::sync::mpsc::UnboundedSender<TripleSubmitOutcome>,
    game: String,
    legs: [OrderIntent; 3],
    ...
    tokio::spawn(async move {
        let ack = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
            backend.submit_triple(&legs[0], &legs[1], &legs[2])
        }))
        .unwrap_or_else(|_| {
            eprintln!("[live] CRITICAL submit_triple task panicked for {game} — reporting a failed basket (game released)");
            exec::TripleAck {
                legs: [
                    Err(exec::ExecError::Rejected("submit_triple panicked".into())),
                    Err(exec::ExecError::Rejected("submit_triple panicked".into())),
                    Err(exec::ExecError::Rejected("submit_triple panicked".into())),
                ],
            }
        });
        let _ = triple_tx.send(TripleSubmitOutcome { game, ack, position, legs, meta, cost_per });
    });
}

/// Apply a SPAWNED basket submission's outcome on the EVENT LOOP's turn. All-3-filled => record the basket
/// (keep the reservation). Otherwise => release the reservation, then NAKED-PAIR RECOVERY: flatten EVERY
/// filled leg (1 or 2 of them) — a 2-of-3 fill is a directional bet (you own 2 outcomes; if the 3rd wins
/// you lose both), a 1-of-3 is a naked single. Any filled leg that can't be flattened => FAIL-CLOSE halt.
#[allow(clippy::too_many_arguments)]
fn apply_triple_outcome(
    backend: &std::sync::Arc<dyn ExecutionBackend>,
    triples_held: &mut std::collections::HashMap<String, TriplePosition>,
    kalshi_books: &std::sync::Arc<std::sync::Mutex<std::collections::HashMap<String, book::KalshiBook>>>,
    pmus_books: &std::collections::HashMap<String, book::PmusBook>,
    exposure: &mut Exposure,
    triple_pending: &mut std::collections::HashSet<String>,
    ...
    outcome_tx: &tokio::sync::mpsc::UnboundedSender<SubmitOutcome>,
    halt: &std::sync::atomic::AtomicBool,
    out: TripleSubmitOutcome,
) {
    triple_pending.remove(&out.game);
    if out.ack.all_filled() {
        // persist each leg's venue order id (positional: ack.legs[i] <-> legs[i]) so a later unwind can cancel.
        let mut pos = out.position;
        for (leg, ack) in pos.legs.iter_mut().zip(out.ack.legs.iter()) {
            if let Ok(a) = ack {
                leg.venue_order_id = a.venue_order_id.clone();
            }
        }
        triples_held.insert(out.game.clone(), pos); // exposure stays RESERVED (keep it)
        println!("[live] TRIPLE LOCKED {} (all 3 legs filled)", out.game);
        return;
    }
    // not all filled -> release the reservation, then recover every FILLED leg.
    release_triple_exposure(exposure, &out.position, out.cost_per);
    if !recover_naked_basket(backend, kalshi_books, pmus_books, flattening, outcome_tx, &out.game, &out.ack, &out.legs, &out.meta) {
        // at least one FILLED leg could not be priced/flattened -> the basket is partially naked -> FAIL-CLOSE.
        triple_naked_failclose(&out.game, &out.ack, halt);
    }
}

/// NAKED-PAIR (basket) RECOVERY — flatten EVERY filled leg of a partially-filled Dutch-book entry. For each
/// of the 3 legs: an `Ok`-but-resting leg with a venue order id is CANCELLED (best-effort, spawned); a leg
/// that FILLED LIVE is FLATTENED with a single marketable SELL read from its live book (reusing the proven
/// single-leg `spawn_flatten` primitive — each flatten routes back as a `SubmitKind::Recovery` outcome that
/// HALTS if the SELL itself doesn't fill). Returns TRUE iff every filled leg was either flattened or has a
/// flatten already in flight; FALSE if ANY filled leg could NOT be priced (one-sided book) — the caller then
/// FAIL-CLOSES (halt) so a filled leg is NEVER left silently un-flattened. FAIL SAFE: priceable filled legs
/// are still flattened even when another filled leg is unpriceable (we never skip a flatten we CAN do).
#[allow(clippy::too_many_arguments)]
fn recover_naked_basket(
    backend: &std::sync::Arc<dyn ExecutionBackend>,
    kalshi_books: &std::sync::Arc<std::sync::Mutex<std::collections::HashMap<String, book::KalshiBook>>>,
    pmus_books: &std::collections::HashMap<String, book::PmusBook>,
    flattening: &mut std::collections::HashSet<String>,
    ...
    let live_filled = |r: &Result<exec::Ack, exec::ExecError>| matches!(r, Ok(a) if a.filled && !a.simulated);
    // if NO leg filled live, there's nothing naked -> recovery is a no-op success (caller's halt is moot).
    if !ack.legs.iter().any(live_filled) {
        return false; // matches the 2-leg convention: no real naked leg -> false -> caller's halt is a no-op
    }
    let mut all_recovered = true;
    // zip the entry legs with their acks + per-outcome static meta (the tag for the coid, the pmus tick).
    for ((leg, leg_ack), oc) in legs.iter().zip(ack.legs.iter()).zip(meta.outcomes.iter()) {
        // the held-leg metadata: a pmus leg carries its tick so the recovery SELL floors to a valid tick (W2).
        let held_leg = PositionLeg {
            venue: leg.venue,
            market: leg.market.clone(),
            side: leg.side,
            venue_order_id: String::new(),
            pm_min_tick: if leg.venue == Venue::Pmus { oc.pm_min_tick } else { None },
        };
        if live_filled(leg_ack) {
            // FILLED leg -> flatten it. Per-leg `flattening` key = the leg's MARKET (so each leg's recovery is
            // tracked independently and a burst can't double-fire a given leg's flatten). If a flatten for this
            // EXACT market is ALREADY in flight (a concurrent 2-leg recovery / postpone-unwind on the same WC
            // outcome market, or a prior basket-recovery attempt), we CANNOT assume it covers THIS basket's
            // filled qty (the other flatten was sized for ITS position) -> FAIL CLOSED rather than trust it.
            if flattening.contains(&leg.market) {
                eprintln!("[live] CRITICAL NAKED BASKET LEG on {game}: filled {:?} {} already has a flatten in flight for a DIFFERENT position (size mismatch risk) -> will HALT", leg.venue, leg.market);
                all_recovered = false; // don't trust another path's flatten to cover this basket -> halt
                continue;
            }
            ...
            let Some(exit) = book.and_then(|b| flatten_exit_cents(&held_leg, &b)) else {
                eprintln!("[live] CRITICAL NAKED BASKET LEG on {game}: filled {:?} {} can't be priced for a flatten (one-sided book) -> will HALT", leg.venue, leg.market);
                all_recovered = false; // this filled leg stays naked -> caller halts; keep flattening the others
                continue;
            };
            let sell = OrderIntent {
                venue: leg.venue,
                ...
                price_cents: exit,
                qty: leg.qty,
                client_order_id: format!("recover3-{}-{}", game, oc.tag.as_char()),
            };
            eprintln!(
                "[live] NAKED BASKET on {game}: leg {:?} {} filled but the basket did not complete -> SELL {}x @ {}c to flatten (no lock recorded).",
                leg.venue, leg.market, sell.qty, sell.price_cents
            );
            flattening.insert(leg.market.clone());
            spawn_flatten(backend, outcome_tx, leg.market.clone(), sell);
        } else if let Ok(a) = leg_ack {
            // an accepted-but-RESTING leg (Ok, filled:false) with an order id -> cancel it (best-effort).
            if !a.filled && !a.venue_order_id.is_empty() {
                let target = exec::CancelTarget { venue: leg.venue, venue_order_id: a.venue_order_id.clone(), market: leg.market.clone() };
                spawn_cancel(backend, game, target);
            }
        }
    }
    all_recovered
...
/// FAIL-CLOSE backstop for a basket where a filled leg could NOT be auto-flattened (one-sided book): log
/// CRITICAL + engage the runtime halt (blocks all new entries) so the un-flattened filled leg surfaces for a
/// MANUAL flatten. Mirrors `naked_leg_failclose` for the 3-leg path. Dry-run acks are simulated+filled ->
/// `all_filled` is always true there, so this never trips in dry-run.
fn triple_naked_failclose(game: &str, ack: &exec::TripleAck, halt: &std::sync::atomic::AtomicBool) {
    halt.store(true, std::sync::atomic::Ordering::Relaxed);
    eprintln!(
        "[live] CRITICAL NAKED BASKET on {game}: a filled leg could not be auto-flattened ({:?}) \
         -> KILL-SWITCH engaged (no new entries). A filled leg is a directional position — flatten MANUALLY.",
        ack.legs
    );
}

/// RESERVE exposure for a basket entry the moment it is SPAWNED (parallel to `reserve_exposure`): bump the
/// per-game / per-cluster / total buckets by `basket_cost * size`. The outcome arm keeps it (records the
/// basket) on an all-filled fill or releases it otherwise.
fn reserve_triple_exposure(exposure: &mut Exposure, pos: &TriplePosition, cost_per: f64) {
    let notional = cost_per * pos.size as f64;
    *exposure.per_pair.entry(pos.game.clone()).or_insert(0.0) += notional;
    *exposure.per_cluster.entry(pos.cluster.clone()).or_insert(0.0) += notional;
    exposure.total += notional;
    exposure.open_positions += 1;
}

/// RELEASE the EXACT basket reservation (the inverse of `reserve_triple_exposure`) for an entry that did not
/// fully fill — subtracts `cost_per * size` from each bucket (never wipes the whole bucket, so a re-entry's
/// own reservation survives), mirroring `release_exposure`.
fn release_triple_exposure(exposure: &mut Exposure, pos: &TriplePosition, cost_per: f64) {
    let notional = cost_per * pos.size as f64;
    if let Some(p) = exposure.per_pair.get_mut(&pos.game) {
        *p = (*p - notional).max(0.0);
    }
    if let Some(c) = exposure.per_cluster.get_mut(&pos.cluster) {
        *c = (*c - notional).max(0.0);
...
fn smoke(cfg: &Config, backend: &dyn ExecutionBackend) {
    ...
    // (6) WORLD-CUP 3-LEG DUTCH BOOK: the FULL-game basket (parallel to the per-outcome binary above). The
    //     game GER/CUW/draw — buy YES on all 3 outcomes, each on its cheapest venue. The 3 cheapest YES asks
    //     are pmus GER 0.42, Kalshi DRAW 0.20, pmus CUW 0.30 = 0.92 basket < $1 -> LOCKED (net of fees + a
    //     small void tail). Exactly one outcome pays $1 -> profit ~ $1 - 0.92. Dry-run fires all 3 YES legs.
    let wc_void_tail = 0.005; // small WC void/postpone tail per $1 (the per-outcome pairs net it implicitly)
    let wc3_meta = TripleMeta {
        game: "ger-cuw-2026-06-14".into(),
        cluster: "fwc-ger-cuw-2026-06-14".into(),
        settle_clean: true,
        days_to_event: Some(1.0),
        outcomes: [
            TripleLeg { tag: OutcomeTag::A, slug: "atc-fwc-ger-cuw-2026-06-14-ger".into(), ticker: "KXWCGAME-26JUN14GERCUW-GER".into(), pm_min_tick: None, pm_min_qty: None },
            TripleLeg { tag: OutcomeTag::D, slug: "atc-fwc-ger-cuw-2026-06-14-draw".into(), ticker: "KXWCGAME-26JUN14GERCUW-TIE".into(), pm_min_tick: None, pm_min_qty: None },
            TripleLeg { tag: OutcomeTag::B, slug: "atc-fwc-ger-cuw-2026-06-14-cuw".into(), ticker: "KXWCGAME-26JUN14GERCUW-CUW".into(), pm_min_tick: None, pm_min_qty: None },
        ],
    };
    // the live quote: per outcome, the cheaper venue's YES ask is the basket leg (GER cheap on pmus, DRAW
    // cheap on Kalshi, CUW cheap on pmus). Depth here is hand-set; the live loop derives it from the books.
    let ocq = |tag: OutcomeTag, pm: Book, k: Book| TripleOutcome { tag, q: OutcomeQuote { tag, pm, k } };
    let wc3 = SoccerTriple {
        game: wc3_meta.game.clone(),
        outcomes: [
            ocq(OutcomeTag::A, Book { yes_bid: Some(0.40), yes_ask: Some(0.42), age_s: 0.2 }, Book { yes_bid: Some(0.44), yes_ask: Some(0.46), age_s: 0.0 }), // GER cheap on pmus (0.42)
            ocq(OutcomeTag::D, Book { yes_bid: Some(0.24), yes_ask: Some(0.26), age_s: 0.2 }, Book { yes_bid: Some(0.18), yes_ask: Some(0.20), age_s: 0.0 }), // DRAW cheap on Kalshi (0.20)
            ocq(OutcomeTag::B, Book { yes_bid: Some(0.28), yes_ask: Some(0.30), age_s: 0.2 }, Book { yes_bid: Some(0.33), yes_ask: Some(0.35), age_s: 0.0 }), // CUW cheap on pmus (0.30)
        ],
        cluster: wc3_meta.cluster.clone(),
        settle_clean: true,
        depth: Depth { c2: 60, c1: 70, c0: 80 },
        days_to_event: Some(1.0),
    };
    let mut sc6 = cfg.clone();
    sc6.assume_sports_settled = true; // WC settle_clean=true already passes; harmless
    println!("[smoke] world-cup 3-LEG DUTCH BOOK (buy YES on all 3 outcomes, each on its cheapest venue):");
    report_triple(&sc6, &wc3_meta, &wc3, wc_void_tail, backend);
}

/// Smoke reporter for the 3-leg Dutch book: price the basket (`dutch_book`), gate it (`evaluate_triple`),
/// then build + dry-run-fire the three YES legs (one per outcome on its cheapest venue), showing the basket
/// cost < $1. Mirrors `report` for the 2-leg path.
fn report_triple(cfg: &Config, meta: &TripleMeta, q: &SoccerTriple, void_tail: f64, backend: &dyn ExecutionBackend) {
    let sig = signal::dutch_book(&[q.outcomes[0].q, q.outcomes[1].q, q.outcomes[2].q], void_tail);
    println!(
        "  basket_cost={:.2} (<$1 = lockable), net={:.1}c, legs: {}",
        sig.basket_cost,
        sig.net * 100.0,
        sig.legs.iter().enumerate().map(|(i, l)| format!("{}@{:?} {:.2}", meta.outcomes[i].tag.as_char(), l.venue, l.yes_ask)).collect::<Vec<_>>().join(", ")
    );
    match evaluate_triple(cfg, q, &sig, &Exposure::new(), 1000) {
        Ok(a) => {
            println!("  APPROVED basket size={}  (net {:.1}c @ {:.2}c/$-day, basket_cost {:.2})", a.size, a.net * 100.0, a.edge_rate, a.cost_per);
            match build_triple(meta, q, &sig, a.size) {
                Some(legs) => {
                    let _ = backend.submit_triple(&legs[0], &legs[1], &legs[2]);
                }
                None => println!("  (could not price all 3 basket legs)"),
            }
        }
...
mod tests {
    ...
    // ==== 3-LEG DUTCH-BOOK (World Cup) — leg construction + the NAKED-PAIR recovery (the critical part) ====

    /// A 3-outcome WC TripleMeta (GER / draw / CUW) + a live quote where each outcome is cheapest on a
    /// specific venue (A pmus, draw Kalshi, B pmus). The basket buys YES on all 3 at the cheapest venue.
    fn wc_triple() -> (TripleMeta, SoccerTriple) {
        let meta = TripleMeta {
            game: "ger-cuw-2026-06-14".into(),
            cluster: "fwc-ger-cuw-2026-06-14".into(),
            settle_clean: true,
        ...
        let ocq = |tag, pma: f64, ka: f64| TripleOutcome { tag, q: OutcomeQuote { tag, pm: Book { yes_bid: Some(pma - 0.02), yes_ask: Some(pma), age_s: 0.1 }, k: Book { yes_bid: Some(ka - 0.02), yes_ask: Some(ka), age_s: 0.0 } } };
        let q = SoccerTriple {
            game: meta.game.clone(),
            outcomes: [
                ocq(OutcomeTag::A, 0.42, 0.46), // A cheap on pmus (0.42 < 0.46)
                ocq(OutcomeTag::D, 0.26, 0.20), // draw cheap on Kalshi (0.20 < 0.26)
                ocq(OutcomeTag::B, 0.30, 0.35), // B cheap on pmus (0.30 < 0.35)
            ],
            cluster: meta.cluster.clone(),
            settle_clean: true,
            depth: Depth { c2: 50, c1: 50, c0: 50 },
    ...
    /// `build_triple` builds 3 YES legs, each on the venue `dutch_book` chose as cheapest, priced from that
    /// venue's book, with the venue-native market id (pmus slug / Kalshi ticker) + `xarb3-<game>-<tag>` coids.
    #[test]
    fn build_triple_builds_three_cheapest_venue_yes_legs() {
        let (meta, q) = wc_triple();
        let sig = signal::dutch_book(&[q.outcomes[0].q, q.outcomes[1].q, q.outcomes[2].q], 0.005);
        let legs = build_triple(&meta, &q, &sig, 3).unwrap();
        // leg A = YES@pmus(GER slug) @ 42c; leg D = YES@Kalshi(TIE ticker) @ 20c; leg B = YES@pmus(CUW slug) @ 30c.
        assert_eq!((legs[0].venue, legs[0].side, legs[0].price_cents), (Venue::Pmus, Side::Yes, 42));
        assert_eq!(legs[0].market, "atc-fwc-ger-cuw-2026-06-14-ger");
        assert_eq!(legs[0].client_order_id, "xarb3-ger-cuw-2026-06-14-A");
        assert_eq!((legs[1].venue, legs[1].side, legs[1].price_cents), (Venue::Kalshi, Side::Yes, 20));
        assert_eq!(legs[1].market, "KXWCGAME-26JUN14GERCUW-TIE");
        assert_eq!(legs[1].client_order_id, "xarb3-ger-cuw-2026-06-14-D");
        assert_eq!((legs[2].venue, legs[2].side, legs[2].price_cents), (Venue::Pmus, Side::Yes, 30));
        assert_eq!(legs[2].market, "atc-fwc-ger-cuw-2026-06-14-cuw");
        assert_eq!(legs[2].client_order_id, "xarb3-ger-cuw-2026-06-14-B");
        // all three are BUY YES (a Dutch book buys YES on every outcome) and share the basket size.
        assert!(legs.iter().all(|l| l.action == Action::Buy && l.side == Side::Yes && l.qty == 3));
    }

    /// An outcome with no YES ask on its chosen venue makes `build_triple` decline (None) -> the loop skips
    /// rather than fire a partial basket. (The signal would already be no_arb, but this is the leg-level guard.)
    #[test]
    fn build_triple_declines_on_unpriceable_chosen_leg() {
        let (meta, mut q) = wc_triple();
        let sig = signal::dutch_book(&[q.outcomes[0].q, q.outcomes[1].q, q.outcomes[2].q], 0.005);
        // wipe the chosen (cheapest) leg's ask for outcome A (pmus) AFTER pricing -> build can't price it.
        q.outcomes[0].q.pm.yes_ask = None;
        assert!(build_triple(&meta, &q, &sig, 1).is_none(), "a one-sided chosen leg -> skip the whole basket");
    }

    /// `triple_position_from_intents` records the 3 entry legs as a held basket (each leg's venue/market/
    /// side=YES; the pair `market`=game), and the 3-leg unwind SELLs back the exact legs.
    #[test]
    fn triple_position_round_trips_to_unwind() {
        let (meta, q) = wc_triple();
        let sig = signal::dutch_book(&[q.outcomes[0].q, q.outcomes[1].q, q.outcomes[2].q], 0.005);
        let legs = build_triple(&meta, &q, &sig, 2).unwrap();
        let pos = triple_position_from_intents(&meta, &legs);
        assert_eq!(pos.game, "ger-cuw-2026-06-14");
        assert_eq!(pos.size, 2);
        assert_eq!(pos.legs[0], PositionLeg { venue: Venue::Pmus, market: "atc-fwc-ger-cuw-2026-06-14-ger".into(), side: Side::Yes, ..Default::default() });
        assert_eq!(pos.legs[1], PositionLeg { venue: Venue::Kalshi, market: "KXWCGAME-26JUN14GERCUW-TIE".into(), side: Side::Yes, ..Default::default() });
        assert_eq!(pos.legs[2], PositionLeg { venue: Venue::Pmus, market: "atc-fwc-ger-cuw-2026-06-14-cuw".into(), side: Side::Yes, ..Default::default() });
        // the 3-leg unwind SELLs back the exact legs.
        let u = unwind::triple_unwind_orders(&pos, [98, 5, 90]);
        assert!(u.iter().all(|o| o.action == Action::Sell && o.side == Side::Yes));
        assert_eq!(u[0].market, "atc-fwc-ger-cuw-2026-06-14-ger");
    }

    // ---- the NAKED-PAIR (basket) RECOVERY — a silently-un-flattened partial basket is the worst bug ----

    /// Build a 3-leg entry's legs + the held meta from `wc_triple`, at the given size. The 3 markets are
    /// the GER slug / TIE ticker / CUW slug.
    fn triple_entry(size: u32) -> (TripleMeta, [OrderIntent; 3]) {
        let (meta, q) = wc_triple();
        let sig = signal::dutch_book(&[q.outcomes[0].q, q.outcomes[1].q, q.outcomes[2].q], 0.005);
        (meta.clone(), build_triple(&meta, &q, &sig, size).unwrap())
    }

    /// A pmus book quoting a YES bid so a filled pmus leg can be flattened at that bid.
    fn pm_book_with_bid(bid: f64) -> book::PmusBook {
        let mut b = book::PmusBook::new();
        b.apply_snapshot(&[(bid, 500.0)], &[(bid + 0.02, 500.0)]);
        b
    }
    /// A Kalshi book quoting a YES bid (via a YES-side level) so a filled Kalshi leg can flatten at it.
    fn k_book_with_bid(bid: f64) -> book::KalshiBook {
        let mut b = book::KalshiBook::new();
        b.apply_snapshot(&[(bid, 500.0)], &[(1.0 - bid - 0.02, 500.0)]);
        b
    }

    /// THE CRITICAL TEST — a 2-of-3 fill flattens BOTH filled legs. Legs A (pmus) and D (Kalshi) filled
    /// LIVE; leg B (pmus) is resting. Recovery cancels the resting leg + fires a flatten SELL on EACH of the
    /// two filled legs (A and D), routed back as Recovery outcomes. No basket recorded, reservation released,
    /// NOT a bare halt. A 2-of-3 fill is a directional bet (own 2 outcomes; if the 3rd wins you lose both),
    /// so leaving it un-flattened is the worst bug — this proves both legs flatten.
    #[tokio::test(flavor = "multi_thread", worker_threads = 2)]
    async fn basket_two_of_three_fill_flattens_both_filled_legs() {
        let (meta, legs) = triple_entry(2);
        let game = meta.game.clone();
        let mut triples_held: std::collections::HashMap<String, TriplePosition> = std::collections::HashMap::new();
        let mut exp = Exposure::new();
        let mut tpending: std::collections::HashSet<String> = std::collections::HashSet::new();
        let mut flat: std::collections::HashSet<String> = std::collections::HashSet::new();
        let halt = AtomicBool::new(false);
        let pos = triple_position_from_intents(&meta, &legs);
        reserve_triple_exposure(&mut exp, &pos, 0.92);
        tpending.insert(game.clone());
        // the FILLED legs' books: leg A (pmus GER slug) + leg D (Kalshi TIE ticker) quote bids to flatten at.
        let mut pmus_books: std::collections::HashMap<String, book::PmusBook> = std::collections::HashMap::new();
        pmus_books.insert(legs[0].market.clone(), pm_book_with_bid(0.40)); // GER (filled)
        let kalshi_books = empty_kbooks();
        lock(&kalshi_books).insert(legs[1].market.clone(), k_book_with_bid(0.18)); // TIE (filled)
        // ack: leg A filled LIVE, leg D filled LIVE, leg B (pmus CUW) resting with an order id (to cancel).
        let ack = exec::TripleAck { legs: [
            Ok(exec::Ack { client_order_id: "A".into(), venue_order_id: "PM-A".into(), filled: true, simulated: false }),
            Ok(exec::Ack { client_order_id: "D".into(), venue_order_id: "K-D".into(), filled: true, simulated: false }),
            Ok(exec::Ack { client_order_id: "B".into(), venue_order_id: "PM-B".into(), filled: false, simulated: false }),
        ] };
        let (otx, _orx) = tokio::sync::mpsc::unbounded_channel::<SubmitOutcome>();
        let out = TripleSubmitOutcome { game: game.clone(), ack, position: pos, legs, meta, cost_per: 0.92 };
        let (rtx, mut rrx) = tokio::sync::mpsc::unbounded_channel::<SubmitOutcome>();
        apply_triple_outcome(&dry_backend(), &mut triples_held, &kalshi_books, &pmus_books, &mut exp, &mut tpending, &mut flat, &rtx, &halt, out);
        drop(otx);
        // NO basket recorded, reservation released, NOT halted, the two filled legs' markets marked flattening.
        assert!(!triples_held.contains_key(&game), "a partial basket records NO lock");
        assert!(exp.total.abs() < 1e-9, "the basket reservation is released");
        assert!(!halt.load(Ordering::Relaxed), "recovery flattens the 2 legs -> does NOT bare-halt");
        assert!(!tpending.contains(&game), "the in-flight marker is cleared");
        assert!(flat.contains("atc-fwc-ger-cuw-2026-06-14-ger"), "the filled pmus leg A is flattening");
        assert!(flat.contains("KXWCGAME-26JUN14GERCUW-TIE"), "the filled Kalshi leg D is flattening");
        // TWO recovery SELLs were fired (one per filled leg) — drain both and confirm they're Recovery + filled.
        let mut recovered = 0;
        while let Ok(Some(o)) = tokio::time::timeout(std::time::Duration::from_secs(2), rrx.recv()).await {
            assert_eq!(o.kind, SubmitKind::Recovery);
            assert!(matches!(&o.ack.a, Ok(a) if a.filled), "each flatten SELL fills in dry-run");
            recovered += 1;
            if recovered == 2 { break; }
        }
        assert_eq!(recovered, 2, "BOTH filled legs of the 2-of-3 fill were flattened");
    }

    /// A 1-of-3 fill flattens the ONE filled leg (a naked single). Only leg D (Kalshi) filled live; A and B
    /// errored (never created an order). Recovery fires exactly one flatten SELL on leg D.
    #[tokio::test(flavor = "multi_thread", worker_threads = 2)]
    async fn basket_one_of_three_fill_flattens_the_single_leg() {
        let (meta, legs) = triple_entry(1);
        let game = meta.game.clone();
        let mut triples_held: std::collections::HashMap<String, TriplePosition> = std::collections::HashMap::new();
        let mut exp = Exposure::new();
        let mut tpending: std::collections::HashSet<String> = std::collections::HashSet::new();
        ...
        reserve_triple_exposure(&mut exp, &pos, 0.92);
        tpending.insert(game.clone());
        // only leg D (Kalshi TIE) filled live -> its book must quote a bid; A and B errored (no order).
        let kalshi_books = empty_kbooks();
        lock(&kalshi_books).insert(legs[1].market.clone(), k_book_with_bid(0.18));
        let pmus_books: std::collections::HashMap<String, book::PmusBook> = std::collections::HashMap::new();
        let ack = exec::TripleAck { legs: [
            Err(exec::ExecError::RateLimited), // A never filled (errored)
            Ok(exec::Ack { client_order_id: "D".into(), venue_order_id: "K-D".into(), filled: true, simulated: false }),
            Err(exec::ExecError::RateLimited), // B never filled (errored)
        ] };
        let out = TripleSubmitOutcome { game: game.clone(), ack, position: pos, legs, meta, cost_per: 0.92 };
        let (rtx, mut rrx) = tokio::sync::mpsc::unbounded_channel::<SubmitOutcome>();
        apply_triple_outcome(&dry_backend(), &mut triples_held, &kalshi_books, &pmus_books, &mut exp, &mut tpending, &mut flat, &rtx, &halt, out);
        assert!(!triples_held.contains_key(&game) && exp.total.abs() < 1e-9);
        assert!(!halt.load(Ordering::Relaxed), "the single filled leg is being flattened -> no bare halt");
        assert!(flat.contains("KXWCGAME-26JUN14GERCUW-TIE"), "the one filled leg (D) is flattening");
        assert!(!flat.contains("atc-fwc-ger-cuw-2026-06-14-ger"), "the errored leg A is NOT flattening (never filled)");
        let o = tokio::time::timeout(std::time::Duration::from_secs(2), rrx.recv()).await.expect("recovery timed out").expect("an outcome was sent");
        assert_eq!(o.kind, SubmitKind::Recovery);
        assert!(rrx.try_recv().is_err(), "exactly ONE flatten SELL for a 1-of-3 fill");
    }

    /// THE FAIL-CLOSE TEST — when a FILLED basket leg CANNOT be priced for a flatten (one-sided book), the
    /// basket is left partially naked, so `apply_triple_outcome` FAILS-CLOSED: engages the runtime halt. The
    /// priceable filled leg is still flattened (never skip a flatten we CAN do); the unpriceable one halts.
    #[tokio::test(flavor = "multi_thread", worker_threads = 2)]
    async fn basket_recovery_failure_fails_closed_with_halt() {
        let (meta, legs) = triple_entry(1);
        let game = meta.game.clone();
        let mut triples_held: std::collections::HashMap<String, TriplePosition> = std::collections::HashMap::new();
        let mut exp = Exposure::new();
        ...
        reserve_triple_exposure(&mut exp, &pos, 0.92);
        tpending.insert(game.clone());
        // BOTH leg A (pmus GER) and leg D (Kalshi TIE) filled live. Leg A's book quotes a bid (priceable),
        // but leg D's Kalshi book is ABSENT -> its flatten can't be priced -> the basket fails closed (halt).
        let mut pmus_books: std::collections::HashMap<String, book::PmusBook> = std::collections::HashMap::new();
        pmus_books.insert(legs[0].market.clone(), pm_book_with_bid(0.40)); // A priceable
        let kalshi_books = empty_kbooks(); // leg D's book absent -> unpriceable
        let ack = exec::TripleAck { legs: [
            Ok(exec::Ack { client_order_id: "A".into(), venue_order_id: "PM-A".into(), filled: true, simulated: false }),
            Ok(exec::Ack { client_order_id: "D".into(), venue_order_id: "K-D".into(), filled: true, simulated: false }),
            Err(exec::ExecError::RateLimited),
        ] };
        let out = TripleSubmitOutcome { game: game.clone(), ack, position: pos, legs, meta, cost_per: 0.92 };
        let (rtx, _rrx) = tokio::sync::mpsc::unbounded_channel::<SubmitOutcome>();
        apply_triple_outcome(&dry_backend(), &mut triples_held, &kalshi_books, &pmus_books, &mut exp, &mut tpending, &mut flat, &rtx, &halt, out);
        // FAIL-CLOSED: the unpriceable filled leg D leaves the basket partially naked -> HALT engaged.
        assert!(halt.load(Ordering::Relaxed), "an unpriceable filled basket leg MUST fail-close (halt)");
        assert!(!triples_held.contains_key(&game), "no lock recorded");
        assert!(exp.total.abs() < 1e-9, "reservation released");
        // the PRICEABLE filled leg A was still flattened (we never skip a flatten we can do).
        assert!(flat.contains("atc-fwc-ger-cuw-2026-06-14-ger"), "the priceable leg A still flattened");
    }

    /// An ALL-3-FILLED basket entry RECORDS the held basket + KEEPS the reservation + persists each leg's
    /// venue order id; no recovery, no halt. (Dry-run acks are simulated+filled -> this is the lock path.)
    #[test]
    fn basket_all_filled_records_and_keeps_reservation() {
        let (meta, legs) = triple_entry(1);
        let game = meta.game.clone();
        let mut triples_held: std::collections::HashMap<String, TriplePosition> = std::collections::HashMap::new();
        let mut exp = Exposure::new();
        ...
        let pos = triple_position_from_intents(&meta, &legs);
        reserve_triple_exposure(&mut exp, &pos, 0.92);
        let reserved = exp.total;
        tpending.insert(game.clone());
        let ack = exec::TripleAck { legs: [
            Ok(exec::Ack { client_order_id: "A".into(), venue_order_id: "PM-A".into(), filled: true, simulated: false }),
            Ok(exec::Ack { client_order_id: "D".into(), venue_order_id: "K-D".into(), filled: true, simulated: false }),
            Ok(exec::Ack { client_order_id: "B".into(), venue_order_id: "PM-B".into(), filled: true, simulated: false }),
        ] };
        let out = TripleSubmitOutcome { game: game.clone(), ack, position: pos, legs, meta, cost_per: 0.92 };
        let (rtx, _rrx) = tokio::sync::mpsc::unbounded_channel::<SubmitOutcome>();
        apply_triple_outcome(&dry_backend(), &mut triples_held, &empty_kbooks(), &std::collections::HashMap::new(), &mut exp, &mut tpending, &mut flat, &rtx, &halt, out);
        let held = triples_held.get(&game).expect("the locked basket is recorded");
        assert_eq!((held.legs[0].venue_order_id.as_str(), held.legs[1].venue_order_id.as_str(), held.legs[2].venue_order_id.as_str()), ("PM-A", "K-D", "PM-B"), "each leg's order id persisted");
        assert!((exp.total - reserved).abs() < 1e-9, "an all-filled basket keeps the reservation");
        assert!(!halt.load(Ordering::Relaxed) && !tpending.contains(&game));
    }

    /// `TripleState` indexes EVERY per-outcome slug AND ticker (6 keys) -> game, and `remove` frees all 6
    /// (so a frame on any of a game's markets finds its triple, and a pruned game leaves no dangling index).
    #[test]
    fn triple_state_indexes_and_frees_all_six_keys() {
        let (meta, _q) = wc_triple();
        let mut ts = TripleState::default();
        ts.insert(meta);
        for key in ["atc-fwc-ger-cuw-2026-06-14-ger", "KXWCGAME-26JUN14GERCUW-GER", "atc-fwc-ger-cuw-2026-06-14-draw", "KXWCGAME-26JUN14GERCUW-TIE", "atc-fwc-ger-cuw-2026-06-14-cuw", "KXWCGAME-26JUN14GERCUW-CUW"] {
            assert_eq!(ts.by_key.get(key).map(String::as_str), Some("ger-cuw-2026-06-14"), "key {key} routes to the game");
        }
        ts.remove("ger-cuw-2026-06-14");
        assert!(ts.by_game.is_empty() && ts.by_key.is_empty(), "remove frees the game + all 6 keys");
    }

    /// `build_quote` returns None until ALL SIX books exist (an incomplete basket is never traded), then
    /// derives the per-outcome cheapest-venue depth as the cross-outcome MIN.
    #[test]
    fn triple_build_quote_needs_all_six_books_then_min_depth() {
        let (meta, _q) = wc_triple();
        let mut pm: std::collections::HashMap<String, book::PmusBook> = std::collections::HashMap::new();
        let mut k: std::collections::HashMap<String, book::KalshiBook> = std::collections::HashMap::new();
        // only some books present -> incomplete -> None.
        pm.insert("atc-fwc-ger-cuw-2026-06-14-ger".into(), pm_book_with_bid(0.40));
        assert!(meta.build_quote(&k, &pm).is_none(), "missing books -> incomplete basket");
        // populate all 6 (3 pmus YES asks + 3 Kalshi). Give the DRAW's cheapest leg (Kalshi) a thin ladder.
        pm.insert("atc-fwc-ger-cuw-2026-06-14-draw".into(), { let mut b = book::PmusBook::new(); b.apply_snapshot(&[(0.24, 50.0)], &[(0.26, 50.0)]); b });
        pm.insert("atc-fwc-ger-cuw-2026-06-14-cuw".into(), { let mut b = book::PmusBook::new(); b.apply_snapshot(&[(0.28, 80.0)], &[(0.30, 80.0)]); b });
        // outcome A: pmus ask 0.42@60 cheaper than Kalshi 0.46 -> A leg depth 60.
        pm.insert("atc-fwc-ger-cuw-2026-06-14-ger".into(), { let mut b = book::PmusBook::new(); b.apply_snapshot(&[(0.40, 60.0)], &[(0.42, 60.0)]); b });
        k.insert("KXWCGAME-26JUN14GERCUW-GER".into(), { let mut b = book::KalshiBook::new(); b.apply_snapshot(&[(0.44, 99.0)], &[(0.52, 99.0)]); b }); // Kalshi A YES ask 1-0.52=0.48 (dearer)
        // outcome draw: Kalshi YES ask 0.20@25 cheaper than pmus 0.26 -> draw leg depth 25 (the THIN leg).
        k.insert("KXWCGAME-26JUN14GERCUW-TIE".into(), { let mut b = book::KalshiBook::new(); b.apply_snapshot(&[(0.16, 5.0)], &[(0.80, 25.0)]); b }); // YES ask = 1 - 0.80 = 0.20 @25
        // outcome B: pmus YES ask 0.30@80 cheaper than Kalshi 0.35 -> B leg depth 80.
        k.insert("KXWCGAME-26JUN14GERCUW-CUW".into(), { let mut b = book::KalshiBook::new(); b.apply_snapshot(&[(0.33, 99.0)], &[(0.65, 99.0)]); b }); // YES ask 1-0.65=0.35 (dearer)
        let q = meta.build_quote(&k, &pm).expect("all 6 books present -> a complete quote");
        // depth = bounded by the THIN draw leg (25): the marginal basket 0.42+0.20+0.30=0.92 (edge .08) lifts
        // min(60,25,80)=25, then the draw leg (one level) exhausts -> the basket can't deepen past 25.
        assert_eq!((q.depth.c2, q.depth.c1, q.depth.c0), (25, 25, 25), "basket depth bounded by the thin draw leg");
    }

    /// `basket_depth` walks the 3 chosen ladders together and STOPS when the marginal basket cost reaches
    /// $1 (edge < 0) — it does NOT just sum each leg's whole ladder (which would over-state depth and let a
    /// basket be sized past the lock). Best levels 0.30+0.30+0.30=0.90 (edge .10, 10 each); deeper levels
    /// 0.34+0.34+0.34=1.02 > $1 -> excluded. So depth = 10 baskets (the best rung only), NOT 10+20=30.
    #[test]
    fn basket_depth_stops_when_marginal_basket_hits_one_dollar() {
        let ladders = [
            vec![(0.30, 10.0), (0.34, 20.0)], // outcome A: best 0.30@10, then 0.34@20
            vec![(0.30, 10.0), (0.34, 20.0)], // outcome D
            vec![(0.30, 10.0), (0.34, 20.0)], // outcome B
        ];
        let d = basket_depth(&ladders);
        // rung1 0.90 edge .10 -> +10 all buckets. rung2 1.02 edge -.02 < 0 -> STOP. depth = 10 (not 30).
        assert_eq!((d.c2, d.c1, d.c0), (10, 10, 10), "depth stops at the lock boundary, not the full ladder");
        // a leg with NO depth -> the whole basket has 0 depth (all 3 outcomes required).
        let one_empty = [vec![(0.30, 10.0)], Vec::new(), vec![(0.30, 10.0)]];
        let e = basket_depth(&one_empty);
        assert_eq!((e.c2, e.c1, e.c0), (0, 0, 0), "an empty leg -> 0 basket depth");
        // the 2c bucket is tighter than the 0c bucket when a deeper rung is positive-but-below-2c: best
        // 0.30*3=0.90 (edge .10, both buckets) then 0.33*3=0.99 (edge .01, only c1/c0) — c2=10, c1=c0=20.
        let stepped = [vec![(0.30, 10.0), (0.33, 10.0)], vec![(0.30, 10.0), (0.33, 10.0)], vec![(0.30, 10.0), (0.33, 10.0)]];
        let s = basket_depth(&stepped);
        assert_eq!((s.c2, s.c1, s.c0), (10, 20, 20), "the 2c bucket excludes the sub-2c-edge deeper rung");
    }
}
