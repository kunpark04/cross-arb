...
# --- scale-in / re-entry (initial clip now sizes UP TO MAX_CONTRACTS_PER_PAIR; adds stack within the per-pair cap) ---
...
# --- exposure caps (per-venue ~$250 wallets) — MAX_NOTIONAL_PER_PAIR=3 makes 3 ctr the HARD per-pair ceiling (incl. scale-in adds) ---
...
echo "[run-live] LIVE prod, ${MAX_CONTRACTS_PER_PAIR}ctr/pair (hard cap), slug-cap=${MAX_POSITIONS_PER_SLUG}, floor=${EDGE_FLOOR_CENTS}c, sports=${ASSUME_SPORTS_SETTLED}, econ=${ASSUME_ECON_SETTLED}, pmus armed — launching" >&2
